...
<?php $__env->startSection('subtitle', 'Update package and status for ' . $pesantren->nama); ?>

<?php $__env->startSection('content'); ?>
<div class="max-w-2xl mx-auto">
    <div class="bg-white rounded-2xl shadow-sm border border-slate-100 p-8">
        <form action="<?php echo e(route('owner.pesantren.update', $pesantren->id)); ?>" method="POST" class="space-y-6">
            <?php echo csrf_field(); ?>
            ...
            <div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <label class="relative cursor-pointer group">
                        <input type="radio" name="package" value="basic" class="peer sr-only" <?php echo e($pesantren->package == 'basic' ? 'checked' : ''); ?>>
                    ...
                    <!-- Advance -->
                    <label class="relative cursor-pointer group">
                        <input type="radio" name="package" value="advance" class="peer sr-only" <?php echo e($pesantren->package == 'advance' ? 'checked' : ''); ?>>
                        <div class="p-4 rounded-xl border-2 border-slate-200 peer-checked:border-indigo-500 peer-checked:bg-indigo-50/50 transition-all hover:border-slate-300">
                    ...
                    <!-- Enterprise -->
                    <label class="relative cursor-pointer group">
                        <input type="radio" name="package" value="enterprise" class="peer sr-only" <?php echo e($pesantren->package == 'enterprise' ? 'checked' : ''); ?>>
                        <div class="p-4 rounded-xl border-2 border-slate-200 peer-checked:border-indigo-500 peer-checked:bg-indigo-50/50 transition-all hover:border-slate-300">
            ...
            <div>
                ...
                <input type="date" name="expired_at" value="<?php echo e($pesantren->expired_at ? \Carbon\Carbon::parse($pesantren->expired_at)->format('Y-m-d') : ''); ?>" class="w-full px-4 py-2 border border-slate-200 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none transition-shadow text-slate-700">
            ...
            <div id="bank-section" class="hidden bg-indigo-50 border border-indigo-100 rounded-xl p-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" name="bank_name" value="<?php echo e(old('bank_name', $pesantren->bank_name)); ?>" placeholder="Contoh: BCA, BSI" class="w-full px-3 py-2 border border-indigo-200 rounded-lg text-sm focus:ring-2 focus:ring-indigo-500 outline-none">
                    </div>
                    <div>
                            <label class="block text-xs font-semibold text-indigo-700 mb-1">No. Rekening</label>
                            <input type="text" name="account_number" value="<?php echo e(old('account_number', $pesantren->account_number)); ?>" class="w-full px-3 py-2 border border-indigo-200 rounded-lg text-sm focus:ring-2 focus:ring-indigo-500 outline-none">
                    </div>
                    <div class="md:col-span-2">
                            <label class="block text-xs font-semibold text-indigo-700 mb-1">Atas Nama</label>
                            <input type="text" name="account_name" value="<?php echo e(old('account_name', $pesantren->account_name)); ?>" class="w-full px-3 py-2 border border-indigo-200 rounded-lg text-sm focus:ring-2 focus:ring-indigo-500 outline-none">
                    </div>
                </div>
                ...
                document.addEventListener('DOMContentLoaded', function() {
                    ...
                    function toggleBank() {
                        ...
                        if(selected === 'advance' || selected === 'enterprise') {
                            ...
                            if ('<?php echo e($pesantren->bank_name); ?>' === '') {
            ...
            <div>
                ...
                <select name="status" class="w-full px-4 py-2 border border-slate-200 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none transition-shadow text-slate-700 bg-white">
                    <option value="active" <?php echo e($pesantren->status == 'active' ? 'selected' : ''); ?>>Active (Allow Access)</option>
                    <option value="suspended" <?php echo e($pesantren->status == 'suspended' ? 'selected' : ''); ?>>Suspended (Block Access)</option>
                </select>
            </div>

            <!-- Actions -->
            <div class="pt-6 border-t border-slate-100 flex items-center justify-end gap-3">
                <a href="<?php echo e(route('owner.pesantren.show', $pesantren->id)); ?>" class="px-5 py-2.5 text-slate-600 font-medium hover:bg-slate-50 rounded-lg transition-colors">
                    Cancel
                </a>
...
<?php echo $__env->make('owner.layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\v\.gemini\antigravity\scratch\santrix\resources\views\owner\pesantren\edit.blade.php ENDPATH**/ ?>